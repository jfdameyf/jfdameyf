        ...
        if level_type == 'SUP':
            ...
            if current_state == 'WATCHING':
                if current_price < level_price:
                    ...
                    logging.info(f"Monitor {key}{flip_tag}: VIOLATED (extension: {violation_dist:.2f})")
                ...
                # RECLAIM TRIGGER
                    ...
                    if monitor['extension'] >= MIN_EXTENSION:
                        ...
                        flip_tag = " [FLIP]" if monitor.get('is_flip', False) else ""
                        logging.info(f"SIGNAL - Monitor {key}{flip_tag}: TRIGGERED! (extension: {monitor['extension']:.2f})")
