"""
ES Trading Bot Backtester - COMPLETE VERSION
...
# ==============================================================================
# BACKTESTER WITH MFE/MAE ANALYSIS
# ==============================================================================
class TradingBotBacktester:
    def __init__(self, start_date, end_date, enable_poc_filter=False,
                 signal_cooldown_minutes=30, target_points=4.0, stop_points=2.0):
        """
        Initialize backtester with trade outcome analysis
        ...
        Args:
            ...
            signal_cooldown_minutes: int - Minimum time between signals (default: 30)
            target_points: float - Profit target in points (default: 4.0)
            stop_points: float - Stop loss in points (default: 2.0)
        """
        ...
        print(f"🔬 BACKTESTER INITIALIZED (COMPLETE VERSION)")
    ...
    def _calculate_trade_outcome(self, signal, bars, entry_idx, entry_time):
        """
        Calculate MFE/MAE and trade outcome
        ...
        return {
            ...
            'mae': round(mae, 2),
            'pnl': round(pnl, 2),
            'pnl_ticks': round(pnl * 4, 0),  # ES = $50/point = $12.50/tick
            'outcome': outcome,
    ...
    def generate_report(self):
        ...
        if not df_trades.empty:
            ...
            print(f"\n   Avg Win: {avg_win:.2f} pts (${avg_win * 50:.0f})")
            print(f"   Avg Loss: {avg_loss:.2f} pts (${avg_loss * 50:.0f})")
            print(f"   Win/Loss Ratio: {abs(avg_win/avg_loss):.2f}" if avg_loss != 0 else "   Win/Loss Ratio: N/A")
            print(f"\n   Total P&L: {total_pnl:.2f} pts (${total_pnl * 50:.0f})")
            print(f"   Avg P&L/Trade: {avg_pnl:.2f} pts (${avg_pnl * 50:.0f})")
            print(f"   Expectancy: ${avg_pnl * 50:.2f} per trade")
        ...
        summary = {
            ...
            'performance': {
                ...
                'total_pnl_dollars': float(total_pnl * 50) if not df_trades.empty else 0,
                'avg_pnl_per_trade': float(avg_pnl) if not df_trades.empty else 0,
                'expectancy_dollars': float(avg_pnl * 50) if not df_trades.empty else 0
            },
            'mfe_mae': {
    ...
    def plot_results(self):
        ...
        fig.suptitle(f'Backtest Results: {self.start_date} to {self.end_date}',
    ...
    def run_full_backtest(self):
        ...
        print("\n" + "="*60)
        print("🚀 STARTING FULL BACKTEST WITH TRADE ANALYSIS")
        print("="*60 + "\n")
...
# ==============================================================================
def quick_backtest(days_back=7, enable_poc=False, cooldown_minutes=30,
                   target_points=4.0, stop_points=2.0):
    """Quick backtest with trade analysis"""
...
if __name__ == "__main__":
    print("="*60)
    print("   ES TRADING BOT BACKTESTER (COMPLETE)")
    print("="*60)
    ...
    cooldown_input = input("Signal Cooldown (minutes) [30]: ").strip()
    cooldown = int(cooldown_input) if cooldown_input else 30

    target_input = input("Target (points) [4.0]: ").strip()
    target = float(target_input) if target_input else 4.0

    stop_input = input("Stop (points) [2.0]: ").strip()
    stop = float(stop_input) if stop_input else 2.0
