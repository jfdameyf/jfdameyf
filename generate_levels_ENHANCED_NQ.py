"""
INSTITUTIONAL LEVEL AUDITOR - ENHANCED VERSION (NQ FUTURES)
Focuses on improving QUALITY of tick-data-derived levels
No theoretical constructs - only objective, evidence-based levels

Adapted for NQ (Nasdaq E-mini futures)
- Symbol: NQ.c.0
- Tick size: 0.25 (same as ES)
- Point value: $20/point (vs ES $50)
- Typical daily range: 150-250 points (4x ES volatility)
"""
...
# Output file
OUTPUT_FILE = "critical_levels_master_enhanced_NQ.csv"
INPUT_FILE = "critical_levels_master_enhanced_NQ.csv"
TICK_SIZE = 0.25
...
LIQUIDITY_MIN_SIZE = 300  # NQ: Slightly lower than ES due to different liquidity profile
...
STRONG_TAIL_VOLUME_THRESHOLD = 8000  # NQ: Adjusted for NQ volume patterns
...
def classify_single_print_strength(block, price_stats, session_high, session_low, block_time):
    ...
    # Factor 4: Time of day (early tails more significant)
    hour = block_time.hour if block_time else 12
    if hour < 11:
        timing_score = 10
...
def identify_persistent_levels(master_df, current_date, lookback_days=10, tolerance=0.5):
    """
...
def detect_liquidity_walls(df_full, min_size, min_duration_minutes):
    ...
    for p, data in walls.items():
        ...
        if dur >= min_duration_minutes:
            ...
            size_score = min(data['max_sz'] / 500, 5)  # Max 5 points from size
            duration_score = min(dur / 30, 5)           # Max 5 points from duration
...
def detect_tpo_single_prints(df):
    """ENHANCED: Now classifies tail strength"""
...
def filter_and_rank_levels(levels_df, persistent_levels, max_levels=12):
    ...
    for persist in persistent_levels:
        mask = (levels_df['price'] >= persist['price'] - 0.5) & \
               (levels_df['price'] <= persist['price'] + 0.5)
...
def process_day_data(day_df, full_df, date_obj, use_l2, master_df):
    ...
    persistent = identify_persistent_levels(master_df, date_obj, lookback_days=10)
...
def main():
    print("=== INSTITUTIONAL LEVEL AUDITOR V20.0 (ENHANCED - NQ) ===")
    print("    SYMBOL: NQ (Nasdaq E-mini Futures)")
    ...
    # Print summary
    print("\n" + "="*60)
    print("LEVEL GENERATION SUMMARY (NQ)")
    print("="*60)
