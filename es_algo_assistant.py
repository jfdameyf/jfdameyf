...
class StrategyManager:
    ...
    def process_trade_tick(self, price: float, size: int, side: str,
        ...
        if candle_1m_now > self.candle_1m_time:
            ...
            triggers.extend(self._check_1m_pre_alerts(current_time))
            self.candle_1m_time = candle_1m_now
            self.candle_1m_delta = 0
        ...
        # --- B. HANDLE 2-MINUTE CANDLE (Main Signals) ---
        ...
        if candle_2m_now > self.candle_2m_time:
            ...
            triggers.extend(self._check_2m_signals(current_time))
            self.candle_2m_time = candle_2m_now
            self.candle_2m_delta = 0
...
class LiveBot:
    ...
    def __init__(self):
        ...
        print(f"   4. Signal Log: {SIGNALS_LOG_FILE}")
    ...
    def _handle_signal(self, trigger: dict) -> None:
        """Process and alert a triggered signal."""
        direction = "LONG" if trigger['type'] == 'SUP' else "SHORT"

        # Console output
        print(f"\n{'='*50}")
        print(f"  {direction} ({trigger['source']}) @ {trigger['price']:.2f}")
            ...
            try:
                ...
                for record in self.live_client:
                    ...
                    try:
                        ...
                        triggers = self.strategy.process_trade_tick(
                            price, size, side, current_time_dt
                        )

                        for trigger in triggers:
                            self._handle_signal(trigger)

                        self.tick_count += 1

                        # Periodic status update
                        if (datetime.now() - self.last_status_time).seconds >= 300:
                            logger.info(f"Active | Ticks: {self.tick_count} | Grid monitors: {len(self.strategy.grid_monitors)}")
                            print(f"[{datetime.now().strftime('%H:%M')}] Active | Ticks: {self.tick_count}")
                            self.last_status_time = datetime.now()
