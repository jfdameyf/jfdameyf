...
SIGNAL_LOG_FILE = f"{LOG_DIR}/gc_signals_{datetime.now().strftime('%Y%m%d')}.csv"
...
class StrategyManager:
    ...
    def detect_level_recapture(self, current_price, level_price, level_type):
        ...
        When a level FAILS (blowout > 10pts):
...
class LiveBot:
    ...
    def send_discord_alert(self, price, order_type, direction, size_mod, message, timestamp):
        ...
        embed = {
            "title": f"🚀 {direction_emoji} GC {direction} SIGNAL",
            ...
            "footer": {"text": "GC Trading Bot • Live Signal"}
        }
